...
namespace supershop
{
    public partial class POSPrintRptShort : Form
    {
        string DeftPrinter = null;
        public POSPrintRptShort(string saleno, string DefaultPrinter)
        {
            InitializeComponent();
        ...
        private void POSPrintRpt_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                string sqlpay = " SELECT * from sales_payment Where TenentID = " + Tenent.TenentID + " and sales_id= '" + toolsaleno.Text + "' and return_id=0 limit 1 ";  
                ...
                if (parameter.autoprintid == "1")
                {
                    if (IsPrinton())
                    {
                        ...
                        string KitchenNot = IsKitchen();
                        if(KitchenNot !="None")
                        {
                            RunKitchen(dtpay, dt, KitchenNot);
                        }
        ...
        public static string IsKitchen()
        {
            string sqlIsbooking = "Select KitchenNotePrinter from tblPrintSetup where TenentID=" + Tenent.TenentID ; //From view combination of tbl_customer and custcredit
            DataTable dt1 = DataAccess.GetDataTable(sqlIsbooking);
        ...
        private void Print()
        {
            ...
            string printerName = DeftPrinter; //use default printer name
        ...
        private void Printkitchen(string kitchenprinter)
        {
            PrinterSettings settings = new PrinterSettings(); //set printer settings
            string printerName = kitchenprinter; //use default printer name
        ...
        private void RunKitchen(DataTable reportds, DataTable reportdetail, string kitchnprinter)
        {
            LocalReport report = new LocalReport();
            var GetDirectory = Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().Location);
            report.ReportPath = GetDirectory + @"\Report\RptPOSShortKitchen.rdlc";
        ...
        private void Run(DataTable reportds, DataTable reportdetail)
        {
            LocalReport report = new LocalReport();
            var GetDirectory = Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().Location);
            report.ReportPath = GetDirectory + @"\Report\RptPOSShort1.rdlc";
            report.DataSources.Add(
               new ReportDataSource("DataSet1", reportds));
            report.DataSources.Add(
               new ReportDataSource("DataSetPrint", reportdetail));
