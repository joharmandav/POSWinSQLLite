...
namespace supershop
{
    public partial class POSPrintRptSplit : Form
    {
        string DeftPrinter = null;
        public POSPrintRptSplit(string InvoiceNO, string DefaultPrinter)
        {
            InitializeComponent();
            toolsaleno.Text = InvoiceNO;
        ...
        private void POSPrintRpt_Load(object sender, EventArgs e)
        {
            try
            {
                string sql = " SELECT  si.sales_id AS salesid, " +
                             ...
                             " Where si.TenentID = " + Tenent.TenentID + " and si.InvoiceNO  = '" + toolsaleno.Text + "' and (sp.return_id=0 or sp.return_id is null) " +
                             " group by item_id ";  
              
                DataTable dt = DataAccess.GetDataTable(sql);

                string sqlpay = " SELECT * from sales_payment Where TenentID = " + Tenent.TenentID + " and InvoiceNO= '" + toolsaleno.Text + "' and return_id=0 ";                
                ...
                ReportDataSource reportDSDetail = new ReportDataSource("DataSetPrintSplit", dt);
                ReportDataSource reportDSPayment = new ReportDataSource("DataSet2", dtpay);
        ...
        private void Run(DataTable reportds, DataTable reportdetail)
        {
            ...
            var GetDirectory = Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().Location);
            report.ReportPath = GetDirectory + @"\Report\RptPOSSplit.rdlc";
