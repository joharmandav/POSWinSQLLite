...
namespace supershop
{
    public partial class POSPrintRptA5 : Form
    {
        string DeftPrinter = null;
        public POSPrintRptA5(string saleno, string DefaultPrinter)
        {
            InitializeComponent();
        ...
        private void POSPrintRpt_Load(object sender, EventArgs e)
        {
            try
            {
                string sql = " SELECT  si.sales_id AS salesid, " +
                             ...
                             " CASE WHEN si.taxapply = 1 THEN 'TX'  ELSE ''  END 'TaxApply', ct.CAT_NAME1 ,si.DeliveryDate ,si.ItemNote,    " +
        ...
        private void Run(DataTable reportds, DataTable reportdetail)
        {
            ...
            var GetDirectory = Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().Location);
            report.ReportPath = GetDirectory + @"\Report\RptPOSA5.rdlc";
