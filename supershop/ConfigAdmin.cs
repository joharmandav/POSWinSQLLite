    ...
    public partial class ConfigAdmin : Form
    {
        ...
        private void Config_Load(object sender, EventArgs e)
        {
            ...
            if (dt1.Rows.Count > 0)
            {
                ...
                chisBooking.Checked = dt1.Rows[0]["IsBooking"].ToString() == "1" ? true : false;
                chIsDelivery.Checked = dt1.Rows[0]["IsDeliveryDt"].ToString() == "1" ? true : false;
                chlPrinting.Checked = dt1.Rows[0]["IsPrint"].ToString() == "1" ? true : false;
                chIsInvoiceWithDelivery.Checked = dt1.Rows[0]["IsInvoiceWithDelivery"].ToString() == "1" ? true : false;
            }
            DefaultValue();
        ...
        private void btnSettingSave_Click(object sender, EventArgs e)
        {
            ...
            int isBooking = chisBooking.Checked ? 1 : 0;
            int isDelivery = chIsDelivery.Checked ? 1 : 0;
            int IsPrint = chlPrinting.Checked ? 1 : 0;
            int IsInvoiceWithDelivery = chIsInvoiceWithDelivery.Checked ? 1 : 0;
                try
                {
                    string Ext = Path.GetExtension(txtLOGO.Text);
                    string imageName = "LOGO" + txtCompanyName.Text + Ext;
                    ...
                    string UploadDate = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff");
                    string sql = " update storeconfig set " +
                                 " IsBooking  = '" + isBooking + "', IsDeliveryDt  = '" + isDelivery + "', IsPrint  = '" + IsPrint + "',IsInvoiceWithDelivery  = '" + IsInvoiceWithDelivery + "', " +
                                 " Uploadby= '" + UserInfo.UserName + "' ,UploadDate = '" + UploadDate + "' ,SynID = 2 " +
                                 " where TenentID= " + Tenent.TenentID + " and id = '" + lblid.Text + "'";

                    DataAccess.ExecuteSQL(sql);//IsInvoiceWithDelivery


                    string sqlwin = " update Win_storeconfig set " +
                                  " IsBooking  = '" + isBooking + "', IsDeliveryDt  = '" + isDelivery + "',IsInvoiceWithDelivery  = '" + IsInvoiceWithDelivery + "', " +
                                 " Uploadby= '" + UserInfo.UserName + "' ,UploadDate = '" + UploadDate + "' ,SynID = 2 " +
                                 " where TenentID= " + Tenent.TenentID + " and id = '" + lblid.Text + "'";
                    Datasyncpso.insert_Live_sync(sqlwin, "Win_storeconfig", "UPDATE");

                    string ActivityName = "Update Store Detail admin Setting";
                    string LogData = "Update Store Detail admin Setting";
                    Login.InsertUserLog(ActivityName, LogData);

                    DialogResult result = MessageBox.Show("Save Successfully. Restart Application?", "Yes or No", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                    if (result == DialogResult.Yes)
                    {
                        this.Close();
                        Application.Restart();
