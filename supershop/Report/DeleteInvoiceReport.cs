...
{
    public partial class DeleteInvoiceReport : Form
    {
        public DeleteInvoiceReport()
        {
            InitializeComponent();
        }
            ...
            else
            {
                try
                {
                    ...
                    string sql = "select sales_id as 'Sales ID', InvoiceNO as 'Recipt No' , (select distinct sales_item.sales_time from sales_item where sales_item.InvoiceNO=sales_payment.InvoiceNO) as Date , SUM(payment_amount) as Total , emp_id as 'Sold by',  " +
                        ...
                        " from sales_payment where Tenentid=" + Tenent.TenentID + " and Deleted = 1 and  sales_time   like  '%" + lblStartDate.Text + "%'  group by InvoiceNO order by sales_id";
                    ...
                    string sql3 = "select SUM(payment_amount), SUM(vat) , SUM(due_amount), SUM(dis) " +
                        " from sales_payment where Tenentid=" + Tenent.TenentID + " and Deleted = 1 and  sales_time like  '%" + lblStartDate.Text + "%' ";
                    DataAccess.ExecuteSQL(sql3);
            ...
            else
            {
                try
                {
                    ...
                    string sql = "select sales_id as 'Sales ID', InvoiceNO as 'Recipt No' , SaleDt as Date , SUM(payment_amount) as Total , emp_id as 'Sold by', " +
                        " sum(dis) as Discount , sum(vat) as TAX , sum(change_amount) as 'change amount', sum(due_amount) as Due, Comment as Comments " +
                        "  from sales_payment where Tenentid=" + Tenent.TenentID + " and Deleted = 1 and  SaleDt BETWEEN '" + startDate + "' AND    '" + endDate + "'  group by InvoiceNO  Order  by sales_id";
                    DataAccess.ExecuteSQL(sql);
                    DataTable dt1 = DataAccess.GetDataTable(sql);
                    datagrdReportDetails.DataSource = dt1;
                    datagrdReportDetails.Columns[0].AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells;
                    ...
                    string sql3 = "select SUM(payment_amount), SUM(vat) , sum(change_amount), SUM(dis)  " +
                                "  from sales_payment where Tenentid=" + Tenent.TenentID + " and Deleted = 1 and  SaleDt BETWEEN '" + startDate + "' AND    '" + endDate + "' Order  by sales_id";
                    DataAccess.ExecuteSQL(sql3);
                    DataTable dt3 = DataAccess.GetDataTable(sql3);
        ...
        private void txtInvoice_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                string sql = "select sales_id as 'Sales ID', InvoiceNO as 'Recipt No' , SaleDt as Date , sum(payment_amount) as Total , emp_id as 'Sold by',  " +
                    " sum(dis) as Discount , sum(vat) as TAX , sum(change_amount) as 'change amount', sum(due_amount) as Due, Comment as Comments " +
                    " from sales_payment where Tenentid=" + Tenent.TenentID + " and Deleted = 1 and    InvoiceNO  Like  '%" + txtInvoice.Text + "%' group by InvoiceNO order by sales_id";
                DataAccess.ExecuteSQL(sql);
                DataTable dt1 = DataAccess.GetDataTable(sql);
                datagrdReportDetails.DataSource = dt1;
                datagrdReportDetails.Columns[0].AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells;
                ...
                string sql3 = "select SUM(payment_amount), SUM(vat) , SUM(due_amount), SUM(dis) " +
                    " from sales_payment  where Tenentid=" + Tenent.TenentID + " and Deleted = 1 and  sales_id  = '" + txtInvoice.Text + "' ";
                DataAccess.ExecuteSQL(sql3);
                DataTable dt3 = DataAccess.GetDataTable(sql3);
