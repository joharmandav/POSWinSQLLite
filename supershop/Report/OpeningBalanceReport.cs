    ...
    public partial class OpeningBalanceReport : Form
    {
        ...
        public void dailyReport()
        {
            ...
            else
            {
                try
                {
                    string sql = " select (select Username from usermgt where TenentID = " + Tenent.TenentID + " and id = DayClose.UserID  ) as 'User' , trmID as 'ShopID', " +
                                  " (select Shift_Name from tbl_Shift where id = DayClose.ShiftID) as 'Shift' , date, " +
                                  " OpAmt as 'Opening Balance' , ShiftCIH as 'Cash in Hand' ,  ShiftSales as 'Shift Sales' ,AdvanceAmt as Advance,ReceivableAmt as Receivable,CreditAmt as Credit, ShiftReturn as 'Shift Return' , " +
                                  " ShiftPurchase as 'Shift Purchase',VoucharAmt as 'vouchar Amount' ,ChequeAmt as 'Cheque Amount', Expamt as 'Expence',  " +
                                  " AMTDelivered as 'Amount Delivered', undeliverdAmt as 'Undelivered Amount', RefNO as 'Reffrence' " +
                                  " from DayClose where TenentID=" + Tenent.TenentID + " and Date like  '%" + lblStartDate.Text + "%' order by Date  ";
        ...
        public void Last30daysReport(string startDate, string endDate)
        {
            ...
            else
            {
                try
                {
                    ...
                    string sql = " select (select Username from usermgt where TenentID=" + Tenent.TenentID + " and id = DayClose.UserID  ) as 'User' , trmID as 'ShopID', " +
                                  " (select Shift_Name from tbl_Shift where id = DayClose.ShiftID) as 'Shift' , date, " +
                                  " OpAmt as 'Opening Balance' , ShiftCIH as 'Cash in Hand' ,  ShiftSales as 'Shift Sales' , ShiftReturn as 'Shift Return' , " +
                                  " ShiftPurchase as 'Shift Purchase',VoucharAmt as 'vouchar Amount' ,ChequeAmt as 'Cheque Amount', Expamt as 'Expence',  " +
                                  " AMTDelivered as 'Amount Delivered', undeliverdAmt as 'Undelivered Amount', RefNO as 'Reffrence' " +
                                  " from DayClose where TenentID=" + Tenent.TenentID + " and Date BETWEEN '" + startDate + "' AND '" + endDate + "' order by Date  ";
