...
namespace supershop
{
    public partial class SpaEmployee_Home : Form
    {
        public SpaEmployee_Home(string a)
        {
            InitializeComponent();
        ...
        private void Driver_Home_Load(object sender, EventArgs e)
        {
            if (Application.OpenForms["AppointmentS1"] != null)
            {
                ...
                Application.OpenForms["AppointmentS1"].WindowState = FormWindowState.Maximized;
            }
            else
            {
        ...
        public void workRecords()
        {
            ...
            string sqlLogInWin = " insert into Win_tbl_workrecords (TenentID,ID, Username, datatype, logdate, logtime, logdatetime,Uploadby ,UploadDate ,SynID) " +
                                 " values (" + Tenent.TenentID + "," + ID + ",'" + UserInfo.UserName + "' , 'OUT' , '" + logdate + "' , " +
                                  " '" + logtime + "' , '" + logdatetime + "','" + UserInfo.UserName + "' , '" + UploadDate + "'  , 1)";
